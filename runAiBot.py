...
if use_AI:
    from modules.ai.connections import create_ai_client, extract_skills, answer_question, close_ai_client, score_job
...
def login_LN() -> None:
    ...
    * If both failed, asks user to login manually
    '''
    # Navigate directly to the plain email/password login page (bypasses Apple/Google SSO buttons)
    driver.get("https://www.linkedin.com/login?fromSignIn=true&trk=guest_homepage-basic_nav-header-signin")
    if username == "username@example.com" and password == "example_password":
        print_lg("User did not configure username and password, hence can't login automatically! Please login manually!")
        manual_login_retry(is_logged_in_LN, 60)
        return
    try:
        email_typed = False
        try:
            print_lg("Waiting for LinkedIn email field...")
            WebDriverWait(driver, 15).until(EC.presence_of_element_located((By.XPATH, "//input[@type='email' or @autocomplete='username' or @id='session_key']")))
            email_fields = driver.find_elements(By.XPATH, "//input[@type='email' or @autocomplete='username' or @id='session_key']")
            for field in email_fields:
                if field.is_displayed():
                    field.clear()
                    field.send_keys(username)
                    email_typed = True
                    print_lg("Email entered successfully.")
                    break
            if not email_typed:
                print_lg("Couldn't find a visible email field.")
        except Exception:
            print_lg("Couldn't find email field within 15 seconds.")
        
        pass_typed = False
        try:
            print_lg("Waiting for LinkedIn password field...")
            WebDriverWait(driver, 15).until(EC.presence_of_element_located((By.XPATH, "//input[@type='password' or @autocomplete='current-password' or @id='session_password']")))
            pass_fields = driver.find_elements(By.XPATH, "//input[@type='password' or @autocomplete='current-password' or @id='session_password']")
            for field in pass_fields:
                if field.is_displayed():
                    ...
                    break
            if not pass_typed:
                print_lg("Couldn't find a visible password field.")
        except Exception:
            print_lg("Couldn't find password field within 15 seconds.")

        if email_typed and pass_typed:
            # Press Enter on the password field – most reliable way to submit any login form
            try:
                for field in pass_fields:
                    if field.is_displayed():
                        field.send_keys(Keys.RETURN)
                        print_lg("Submitted login form via Enter key.")
                        break
            except Exception:
                # Fallback: try clicking a visible Sign in button
                try:
                    sign_in_buttons = driver.find_elements(By.XPATH, '//button[@type="submit" or contains(., "Sign in")]')
                    ...
                    print_lg("Error submitting login form:", e)
        else:
            print_lg("Login skipped – could not fill both fields.")
...
def set_search_location(override: str = "") -> None:
    '''
    Function to set search location. Pass `override` to set a specific location
    instead of the global `search_location` config value.
    '''
    loc = override.strip() if override.strip() else search_location.strip()
    if loc:
        try:
...
def apply_filters(location_override: str = "") -> None:
    '''
    Function to apply job search filters. Pass location_override to use a
    specific location for this combination instead of the global config value.
    '''
    ...
    try:
        ...
        print_lg("Waiting for 'All filters' button...")
        WebDriverWait(driver, 15).until(
            EC.presence_of_element_located((By.XPATH, '//button[normalize-space()="All filters"]'))
        ).click()
        print_lg("Clicked 'All filters'. Applying settings...")
        ...
        if apply_method == "Easy Apply Only": boolean_button_click(driver, actions, "Easy Apply")
        ...
        print_lg("Clicking 'Show results'...")
        ...
        print_lg("Filters applied. Waiting for job listings...")

        global pause_after_filters
        if pause_after_filters:
            print_lg("Paused after filters. Continuing automatically in 10 seconds (change pause_after_filters to False to skip this)...")
            sleep(10)

    except Exception as e:
        print_lg("Setting the preferences failed! Continuing anyway...")
        critical_error_log("Faced error while applying filters.", e)
        sleep(5)
...
def check_blacklist(rejected_jobs: set, job_id: str, company: str, blacklisted_companies: set) -> tuple[set, set, WebElement] | ValueError:
    ...
    log_dom_action("READ", f"About Company:\n{about_company_org}")
    skip_checking = False
...
def get_job_description(
    ...
    try:
        ...
        log_dom_action("READ", f"Job Description:\n{jobDescription}")
                ...
                if answer == "":
                    ...
                    if use_AI and aiClient:
                        try:
                            ...
                            log_dom_action("READ FORM", f"Question (text): {label_org} | AI answered: {ai_answer}")
                            answer = ai_answer or '0'
                        except Exception as e:
                ...
                if answer == "":
                    ...
                    if use_AI and aiClient:
                        try:
                            ...
                            log_dom_action("READ FORM", f"Question (textarea): {label_org} | AI answered: {ai_answer}")
                            answer = ai_answer or '0'
                        except Exception as e:
                            print_lg("Failed to get AI answer!", e)
                    if ai_answer and isinstance(ai_answer, str) and ai_answer.strip():
...
def external_apply(pagination_element: WebElement, job_id: str, job_link: str, resume: str, date_listed, application_link: str, screenshot_name: str, title: str = "", company: str = "", search_term: str = "", search_location: str = "") -> tuple[bool, str, int]:
    '''
    Function to open new tab and save external job application links
    '''
    global tabs_count, dailyEasyApplyLimitReached
    if apply_method == "Easy Apply Only":
        try:
    ...
    try:
        ...
        failed_job(job_id, job_link, resume, date_listed, "Probably didn't find Apply button or unable to switch tabs.", e, application_link, screenshot_name, title, company, search_term, search_location)
        global failed_count
        failed_count += 1
        return True, application_link, tabs_count
...
def failed_job(job_id: str, job_link: str, resume: str, date_listed, error: str, exception: Exception, application_link: str, screenshot_name: str, title: str = "", company: str = "", search_term: str = "", search_location: str = "") -> None:
    '''
    Function to update failed jobs list in excel
    '''
    try:
        with open(failed_file_name, 'a', newline='', encoding='utf-8') as file:
            fieldnames = ['Job ID', 'Title', 'Company', 'Job Link', 'Resume Tried', 'Date listed', 'Date Tried', 'Assumed Reason', 'Stack Trace', 'External Job link', 'Screenshot Name', 'Search Term', 'Search Location']
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            if file.tell() == 0: writer.writeheader()
            record = {
                'Job ID': job_id, 'Title': title, 'Company': company, 'Job Link': job_link, 'Resume Tried': resume,
                ...
                'Search Term': search_term, 'Search Location': search_location,
            }
        ...
        critical_error_log("Failed to update the excel of failed jobs!", e)
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'], 
                   ...
                   questions_list: set | None, connect_request: Literal['In Development'], search_term: str = "", search_location: str = "") -> None:
    '''
    Function to create or update the Applied jobs CSV file, once the application is submitted successfully
    '''
    try:
        with open(file_name, mode='a', newline='', encoding='utf-8') as csv_file:
            fieldnames = ['Job ID', 'Title', 'Company', 'Work Location', 'Work Style', 'About Job', 'Experience required', 'Skills required', 'HR Name', 'HR Link', 'Resume', 'Re-posted', 'Date Posted', 'Date Applied', 'Job Link', 'External Job link', 'Questions Found', 'Connect Request', 'Search Term', 'Search Location']
        ...
        critical_error_log("Failed to update the excel of applied jobs!", e)
...
def discard_job() -> None:
    try:
        ...
        # Try finding Discard inside confirmation modal
        discard_clicked = False
        for xpath in [
            "//div[contains(@data-test-modal-id, 'discard-confirmation')]//button[contains(., 'Discard')]",
            "//button[contains(@class, 'artdeco-modal__confirm-dialog-btn') and contains(., 'Discard')]",
            "//button[contains(span, 'Discard')]",
            ...
            try:
                ...
                if el.is_displayed():
                    ...
                    print_lg("Clicked Discard confirmation button.")
                    break
            except:
                pass
        ...
        print_lg("Error during discard_job:", e)
...
def apply_to_jobs(search_terms: list[str]) -> None:
    ...
    # Build list of locations from comma-separated search_location config.
    # Each entry is stripped; empty string means "no location override" (LinkedIn default).
    raw_locations = [loc.strip() for loc in search_location.split(",") if loc.strip()]
    search_locations = raw_locations if raw_locations else [""]

    # Create all (term, location) combinations
    combos = [(term, loc) for term in search_terms for loc in search_locations]
    if randomize_search_order:
        shuffle(combos)

    # Fail page loads in 60s instead of the default 120s so we recover faster
    try:
    ...
    round_number = 1
    while True:
        print_lg(f"\n==================== Starting Combo Round-Robin Cycle {round_number} ====================\n")
        if randomize_search_order:
            shuffle(combos)

        total_combos = len(combos)
        for combo_idx, (searchTerm, searchLoc) in enumerate(combos, 1):
            loc_label = searchLoc if searchLoc else "(default location)"
        try:
            driver.get(f"https://www.linkedin.com/jobs/search/?keywords={searchTerm}")
        except Exception as _nav_err:
            print_lg(f"⚠️  Page load timed out for combo {combo_idx} ('{searchTerm}' in '{loc_label}'). Skipping to next combo.")
            try:
            ...
            continue
        print_lg("\n" + "="*120 + "\n")
        print_lg(f'>>> Combo {combo_idx}/{total_combos}: "{searchTerm}" in "{loc_label}" <<<\n')

        apply_filters(searchLoc)
        ...
        try:
            while current_count < switch_number:
                ...
                try:
                    ...
                    print_lg(f"[STUCK] No job listings appeared for combo {combo_idx}. Moving to next combination.")
                    if use_AI:
                        _action = handle_stuck(driver, aiClient, "no_job_listings_loaded",
                                               f"Waiting for job listings for '{searchTerm}' in '{loc_label}'", print_lg)
                        execute_stuck_action(driver, _action, actions, print_lg)
                    break
                ...
                for job in job_listings:
                    ...
                    try:
                        ...
                        print_lg(f"Skipping job card due to error reading details: {je}")
                        continue
                    ...
                    try:
                        ...
                        failed_job(job_id, job_link, resume, date_listed, "Found Blacklisted words in About Company", e, "Skipped", screenshot_name, title, company, searchTerm, searchLoc)
                        rejected_jobs.add(job_id)
                        skip_count += 1
                        continue
                    except Exception as e:
                    ...
                    if skip:
                        print_lg(message)
                        failed_job(job_id, job_link, resume, date_listed, reason, message, "Skipped", screenshot_name, title, company, searchTerm, searchLoc)
                        rejected_jobs.add(job_id)
                        skip_count += 1
                        continue
                    ...
                    if use_AI and description != "Unknown":
                        try:
                            ...
                            if _score != -1 and _score < _threshold:
                                msg = f"[SCORE {_score:3d}/100] ⏭ SKIPPED — {_score_reason}"
                                print_lg(msg)
                                failed_job(job_id, job_link, resume, date_listed, "AI Score Too Low", Exception(msg), "Skipped", screenshot_name, title, company, searchTerm, searchLoc)
                                rejected_jobs.add(job_id)
                                skip_count += 1
                                continue
                            elif _score != -1:
                                print_lg(f"[SCORE {_score:3d}/100] ✅ Proceeding — {_score_reason}")
                        except Exception as _se:
                            print_lg("Job scoring error (skipping score filter):", _se)
                    ...
                    if not is_easy_apply:
                        try:
                            ...
                            pass
                    if is_easy_apply:
                        if apply_method == "External Only":
                            print_lg("Job is Easy Apply, but apply_method is External Only. Skipping.")
                            failed_job(job_id, job_link, resume, date_listed, "Skip Easy Apply", Exception("apply_method is External Only"), "Skipped", screenshot_name, title, company, searchTerm, searchLoc)
                            rejected_jobs.add(job_id)
                            skip_count += 1
                            try: actions.send_keys(Keys.ESCAPE).perform()
                            except: pass
                        ...
                        try: 
                            try:
                                ...
                                while next_button:
                                    ...
                                    if next_counter >= 15: 
                                        if pause_at_failed_question:
                                            ...
                                            print_lg("Couldn't answer one or more questions. Pausing for 20 seconds. You can turn off 'Pause at failed question' in config.py")
                                            sleep(20)
                                            next_counter = 1
                                            continue
                                        ...
                                        errored = "stuck"
                                        # Ask LLM how to recover, use cache for next time
                                        try:
                                            from modules.stuck_handler import handle_stuck, execute_stuck_action
                                            _stuck_action = handle_stuck(
                                                driver, aiClient if use_AI else None,
                                                "modal_stuck_on_questions",
                                                f"Bot stuck in question loop for job '{title}' at '{company}'. Questions seen: {questions_list}",
                                                print_lg
                                            )
                                            execute_stuck_action(driver, _stuck_action, actions, print_lg)
                                        except Exception as _sh_err:
                                            print_lg("Stuck handler error:", _sh_err)
                                ...
                                if errored != "stuck" and cur_pause_before_submit:
                                    print_lg('Pausing for 30 seconds for manual review before submitting... (Turn off "Pause before submit" in settings to disable this).')
                                    time.sleep(30)
                            ...
                            failed_job(job_id, job_link, resume, date_listed, "Problem in Easy Applying", e, application_link, screenshot_name, title, company, searchTerm, searchLoc)
                            failed_count += 1
                            discard_job()
                            continue
                    else:
                        if apply_method == "Easy Apply Only":
                            print_lg("Job is External, but apply_method is Easy Apply Only. Skipping.")
                            failed_job(job_id, job_link, resume, date_listed, "Skip External Apply", Exception("apply_method is Easy Apply Only"), "Skipped", screenshot_name, title, company, searchTerm, searchLoc)
                            rejected_jobs.add(job_id)
                            skip_count += 1
                            continue

                        # Case 2: Apply externally
                        skip, application_link, tabs_count = external_apply(pagination_element, job_id, job_link, resume, date_listed, application_link, screenshot_name, title, company, searchTerm, searchLoc)
                        if dailyEasyApplyLimitReached:
                            print_lg("\n###############  Daily application limit for Easy Apply is reached!  ###############\n")
                            return
                        if skip: continue

                    submitted_jobs(job_id, title, company, work_location, work_style, description, experience_required, skills, hr_name, hr_link, resume, reposted, date_listed, date_applied, job_link, application_link, questions_list, connect_request, searchTerm, searchLoc)
...
def main() -> None:
    ...
    try:
        ...
        if not os.path.exists(default_resume_path):
            print_lg(f'Your default resume "{default_resume_path}" is missing! For now the bot will continue using your previous upload from LinkedIn!')
            useNewResume = False
        ...
        err_str = str(e)
        if "Read timed out" in err_str or "HTTPConnectionPool" in err_str or "timed out" in err_str.lower():
            print_lg("⚠️  Page load timed out (browser/network too slow). Attempting to recover...")
            try:
                driver.set_page_load_timeout(60)
                driver.get("https://www.linkedin.com/feed/")
                print_lg("Recovered — restarting job search...")
                total_runs = run(total_runs)
            except Exception as recover_err:
                print_lg("Recovery failed, exiting:", recover_err)
        else:
            print_lg("The browser window was closed or the session became invalid. Exiting.", e)
    except Exception as e:
        err_str = str(e)
        if "Read timed out" in err_str or "HTTPConnectionPool" in err_str:
            print_lg("⚠️  Selenium HTTP timeout — LinkedIn page took too long to respond.")
            print_lg("Tip: Close Chrome windows before starting, or check your internet connection.")
        else:
            print_lg(alert_title, e)
    finally:
