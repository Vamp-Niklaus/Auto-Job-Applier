...
def createChromeSession(isRetry: bool = False):
    ...
    if isRetry:
        ...
        if os.path.lexists(os.path.join(profile_dir, "SingletonLock")) or os.path.lexists(os.path.join(profile_dir, "lockfile")):
...
try:
    ...
    print_lg("\nChrome is open in the background! Bypassing your main profile and logging in with a fresh Guest Profile...")
    try:
        options, driver, actions, wait = createChromeSession(True)
    except Exception as e2:
        print_lg("Failed to start guest profile as well!", e2)
        raise e2
